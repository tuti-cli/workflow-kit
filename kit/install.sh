...
read_github_config() {
    ...
    if [ ! -f "$claude_md" ]; then
        log_warning "CLAUDE.md not found — GitHub config must be set manually"
        GITHUB_OWNER=""
        GITHUB_REPO_NAME=""
...
create_directories() {
    ...
    mkdir -p "$PROJECT_ROOT/.claude/commands/rules"
    ...
    mkdir -p "$PROJECT_ROOT/.workflow/audits"
    ...
    touch "$PROJECT_ROOT/.workflow/audits/.gitkeep"
...
install_components() {
    ...
    # Install rules commands (v2)
    if [ -d "$KIT_DIR/kit/.claude/commands/rules" ]; then
        mkdir -p "$PROJECT_ROOT/.claude/commands/rules"
        for f in "$KIT_DIR/kit/.claude/commands/rules/"*.md; do
            [ -f "$f" ] || continue
            name=$(basename "$f")
            dest="$PROJECT_ROOT/.claude/commands/rules/$name"
            backup_existing_file "$dest"
            install_file "$f" "$dest"
            count=$((count + 1))
        done
    ...
    count=0
    for rules_dir in "$KIT_DIR/kit/.claude/rules/"*/; do
        [ -d "$rules_dir" ] || continue
        local rules_name
        rules_name=$(basename "$rules_dir")
        mkdir -p "$PROJECT_ROOT/.claude/rules/$rules_name"
        for f in "$rules_dir"*.md; do
            [ -f "$f" ] || continue
            name=$(basename "$f")
            dest="$PROJECT_ROOT/.claude/rules/$rules_name/$name"
            backup_existing_file "$dest"
            install_file "$f" "$dest"
            count=$((count + 1))
        done
    ...
    # Install .workflow templates (v2)
    log_info "Installing workflow templates..."
    count=0
    for f in "$KIT_DIR/kit/.workflow/"*.md; do
        [ -f "$f" ] || continue
        name=$(basename "$f")
        dest="$PROJECT_ROOT/.workflow/$name"
        if [ ! -f "$dest" ]; then
            install_file "$f" "$dest"
            count=$((count + 1))
        fi
    done
    # Install .workflow subdirectories
    for subdir in ADRs patches features; do
        if [ -d "$KIT_DIR/kit/.workflow/$subdir" ]; then
            mkdir -p "$PROJECT_ROOT/.workflow/$subdir"
            for f in "$KIT_DIR/kit/.workflow/$subdir/"*.md; do
                [ -f "$f" ] || continue
                name=$(basename "$f")
                dest="$PROJECT_ROOT/.workflow/$subdir/$name"
                if [ ! -f "$dest" ]; then
                    install_file "$f" "$dest"
                    count=$((count + 1))
                fi
    ...
    log_success "Installed workflow templates"
...
update_components() {
    ...
    for section in "agents" "commands/workflow" "commands/agents" "commands/rules" "rules/base" "rules/project" "rules/features"; do
...
print_install_summary() {
    ...
    echo "  2. Run: /ww:discover                (analyzes project, recommends agents)"
    echo "  3. Run: /ww:plan                    (plan your first feature)"
